            ...
            <div className="mt-8 space-y-3">
              {[copy.auth.benefit1, copy.auth.benefit2, copy.auth.benefit3].map((item) => (
                <div key={item} className="auth-benefit">
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-400/10 text-emerald-300"><CheckIcon className="h-4 w-4" /></span>
        ...
        <section className="auth-form-side">
          <div className="w-full max-w-lg fade-up">
            <div className="mb-7 flex items-center justify-between gap-3 sm:mb-8">
              <div className="min-w-0 lg:hidden"><BrandLogo /></div>
              <Link href="/" className="hidden text-sm font-semibold text-slate-400 transition hover:text-emerald-400 lg:inline-flex">{copy.auth.home}</Link>

              <div className="ms-auto flex shrink-0 items-center gap-2">
                <button type="button" onClick={toggleLocale} className="preference-button" aria-label={copy.common.language} title={copy.common.language}>
                  <LanguagesIcon className="h-4 w-4" />
                  <span>{locale === "en" ? "AR" : "EN"}</span>
                </button>
                <button type="button" onClick={toggleTheme} className="preference-button" aria-label={copy.nav.theme} title={copy.nav.theme}>
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 text-xs text-slate-500 sm:grid-cols-2">
              <div className="auth-mini-proof"><ShieldIcon className="h-4 w-4 shrink-0 text-emerald-400" />{copy.auth.secureSession}</div>
              <div className="auth-mini-proof"><CalendarIcon className="h-4 w-4 shrink-0 text-sky-400" />{copy.auth.easyBooking}</div>
            </div>
          </div>
        </section>
      </div>
