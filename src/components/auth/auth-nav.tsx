  ...
  if (isLoading) {
    return (
      <div className="session-pill">
        ...
        <span className="hidden sm:inline">{copy.nav.checkingSession}</span>
      </div>
    );
  }
  ...
  if (!user) {
    return (
      <div className="flex items-center gap-2">
        <Link href="/login" className="secondary-button min-h-10 px-3.5 py-2 text-xs sm:px-4 sm:text-sm">{copy.common.login}</Link>
        <Link href="/register" className="primary-button hidden min-h-10 px-3.5 py-2 text-xs sm:inline-flex sm:px-4 sm:text-sm">{copy.common.getStarted}</Link>
      </div>
    );
  }
    ...
    <div className="group relative">
      ...
      <div className="user-menu">
        ...
        <Link href="/dashboard" className="user-menu-link">
          <LayoutDashboardIcon className="h-4 w-4 text-sky-400" />{copy.common.dashboard}
        </Link>
        <Link href="/appointments" className="user-menu-link">
          <CalendarIcon className="h-4 w-4 text-emerald-400" />{copy.nav.myAppointments}
        </Link>
        <button type="button" onClick={handleLogout} disabled={isLoggingOut} className="user-menu-link w-full text-red-300 hover:bg-red-500/10">
          <LogOutIcon className="h-4 w-4" />{isLoggingOut ? copy.nav.loggingOut : copy.common.logout}
        </button>
      </div>
    </div>
  );
